...
def test_create_project_with_admin_user_calls_post_and_put(
    client, mock_artifactory_client
):
...
def test_create_project_with_admin_group_calls_post_and_put(
    client, mock_artifactory_client
):
    payload = {
        **VALID_PROJECT_PAYLOAD,
        "spec": {
            **VALID_PROJECT_PAYLOAD["spec"],
            "admin_user": None,
...
def test_create_project_with_both_admin_user_and_group_calls_two_puts(
    client, mock_artifactory_client
):
    payload = {
        **VALID_PROJECT_PAYLOAD,
        "spec": {**VALID_PROJECT_PAYLOAD["spec"], "admin_group": "devops-team"},
    }
    client.post(f"{PREFIX}/", json=payload)
...
def test_create_project_external_error_returns_error_response(mock_artifactory_client):
    mock_artifactory_client.post = AsyncMock(
        return_value=make_response(409, {"message": "already exists"})
    )
    app = FastAPI()
...
def test_create_project_unexpected_exception_triggers_rollback(mock_artifactory_client):
    ...
    mock_artifactory_client.put = AsyncMock(
...
def test_grant_permission_user_only_calls_put(client, mock_artifactory_client):
    ...
    assert mock_artifactory_client.put.call_count == 1
    assert (
        "projects/my-project/users/alice"
        in mock_artifactory_client.put.call_args.args[0]
    )
...
def test_grant_permission_existing_group_returns_200():
    ...
    response = TestClient(app).post(
        f"{PREFIX}/permissions", json=VALID_PERMISSION_GROUP_PAYLOAD
    )
    assert response.status_code == 200
    assert response.json()["status"] == "successful"
...
def test_get_permissions_returns_200():
    ...
    c.get = AsyncMock(
        side_effect=[make_response(200, users), make_response(200, groups)]
    )
    app = FastAPI()
    app.include_router(get_v1_artifactory_router(c))
...
@patch("app.helpers.httpx.AsyncClient")
def test_xray_vuln_update_fetches_from_s3_then_posts_to_xray(
    mock_httpx, client, mock_artifactory_client
):
    mock_httpx.return_value = _make_s3_mock()
...
@patch("app.helpers.httpx.AsyncClient")
def test_xray_vuln_update_xray_error_returns_error_response(
    mock_httpx, mock_artifactory_client
):
    mock_httpx.return_value = _make_s3_mock()
    mock_artifactory_client.post = AsyncMock(return_value=make_response(500))
    app = FastAPI()
