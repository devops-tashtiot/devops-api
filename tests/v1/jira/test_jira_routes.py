...
def test_create_project_with_both_assigns_user_and_group(client, mock_jira_client):
    ...
    role_calls = [
        c for c in mock_jira_client.post.call_args_list if "role" in c.args[0]
    ]
...
def test_create_project_error_returns_error_response(mock_jira_client):
    mock_jira_client.post = AsyncMock(
...
def test_create_project_already_exists_returns_400(mock_jira_client):
    ...
    conflict.json = MagicMock(
        return_value={"errorMessages": ["A project with that name already exists."]}
    )
...
def test_create_project_nonexistent_admin_user_rejected_before_create(mock_jira_client):
    ...
    not_found.json = MagicMock(
        return_value={"errorMessages": ["The user named 'admin' does not exist"]}
    )
...
def test_create_project_nonexistent_admin_group_rejected_before_create(
    mock_jira_client,
):
    ...
    not_found = MagicMock(status_code=404, text="")
    not_found.json = MagicMock(
        return_value={"errorMessages": ["The group named 'dev-team' does not exist"]}
    )
    mock_jira_client.get = AsyncMock(return_value=not_found)
...
def test_delete_project_not_found_returns_404(mock_jira_client):
    mock_jira_client.delete = AsyncMock(
