...
VALID_PAYLOAD = {
    ...
    "spec": {"plugin_name": "my-plugin-1.0.jar"},
}
...
def test_install_plugin_uploads_to_upm_with_token(
...
def test_install_plugin_sends_correct_filename(
    client, mock_s3_http, mock_confluence_client
):
    client.post(f"{PREFIX}/plugin/", json=VALID_PAYLOAD)
...
def test_install_plugin_poll_follows_redirects(
    client, mock_s3_http, mock_confluence_client
):
...
def test_install_plugin_task_error_returns_422(
    client, mock_s3_http, mock_confluence_client
):
    failed = MagicMock(
        status_code=202,
...
def test_install_plugin_never_done_returns_504(mock_s3_http, monkeypatch):
    ...
    mock_confluence_client.get = AsyncMock(
        side_effect=[token_response, never_done, never_done]
    )
