...
from .operations import (
    ...
    create_project,
    delete_project,
    fetch_vuln_update_from_s3,
    get_global_role,
    get_project_permissions,
    increase_storage_quota,
    upload_xray_vulnerability_update,
)
from .schemas import (
    ArtifactoryPermissionRequest,
    ArtifactoryProjectRequest,
    ArtifactoryStorageQuotaRequest,
    ArtifactoryXrayUpdateRequest,
)
...
def get_v1_artifactory_router(artifactory_client: Any):
    ...
    @router.post(
        "/storage-quota", summary="Increase project storage quota", status_code=201
    )
    ...
    @router.get(
        "/permissions/roles/{role_name}", name="get global role", status_code=200
    )
    ...
    @router.post(
        "/permissions",
        name="grant project permission to user or group",
        status_code=200,
    )
    ...
    @router.get(
        "/permissions/{project_key}",
        name="get all permissions for a project",
        status_code=200,
    )
    ...
    @router.post(
        "/xray/vulnerability-update",
        name="upload Xray air-gapped vulnerability update",
        status_code=200,
    )
    async def xray_vulnerability_update(
        payload: ArtifactoryXrayUpdateRequest,
    ) -> JSONResponse:
        try:
            file_bytes = await fetch_vuln_update_from_s3(payload.spec.file_name)
            await upload_xray_vulnerability_update(
                artifactory_client, file_bytes, payload.spec.file_name
            )
            return SuccessResponse(status="successful")
        except HTTPException as external_error:
            return JSONResponse(
