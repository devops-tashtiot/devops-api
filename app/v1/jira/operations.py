...
async def create_project(jira_client: Any, payload: ProjectSpec) -> None:
    ...
    try:
        ...
        logger.error(f"Unexpected error creating project {payload.key}: {e!s}")
        raise
...
async def delete_project(jira_client: Any, payload: ProjectSpec) -> None:
    ...
    try:
        ...
        logger.error(f"Unexpected error deleting project {payload.key}: {e!s}")
        raise
...
async def assign_project_admin_user(jira_client: Any, payload: ProjectSpec) -> None:
    ...
    try:
        ...
        logger.error(
            f"Unexpected error assigning admin user to project {payload.key}: {e!s}"
        )
        raise
...
async def assign_project_admin_group(jira_client: Any, payload: ProjectSpec) -> None:
    ...
    try:
        response = await jira_client.post(
            endpoint, json={"group": [payload.admin_group]}
        )
        _handle_response(response)
    except Exception as e:
        logger.error(
            f"Unexpected error assigning admin group to project {payload.key}: {e!s}"
        )
        raise
