...
once the space is actually removed — a `GET /space/{key}` immediately after a "successful" delete
still returns the full space (`200`); it only starts 404ing ~20-30s later. `delete_space` polls
`GET /space/{key}` (reusing `CONFLUENCE_JOB_POLL_INTERVAL`/`CONFLUENCE_JOB_MAX_POLLS`, same as the
export/import job-polling below) until it 404s before reporting success, raising `504` if it
never confirms within the timeout. Without this, a caller recreating a space with the same key
right after a "successful" delete could race the background job.
...
**UPM install is asynchronous.** Step 3's response is `202`, not a completed result:
`{"status":{"done":false,...},"links":{"self":"/rest/plugins/1.0/pending/{taskId}"}}`. Reporting
success right after step 3 is dishonest — a caller checking whether the plugin exists
immediately afterward can get a `404` because the OSGi bundle hasn't finished registering, even
though devops-api already said `"successful"`. `install_plugin` polls `GET` on the task's
`links.self` (reusing `CONFLUENCE_JOB_POLL_INTERVAL`/`CONFLUENCE_JOB_MAX_POLLS`) until
`status.done` is `true`, raising `422` if `status.contentType` contains `"err"` (a genuine
install failure, with `status.errorMessage` as the detail — e.g. a corrupt/invalid jar), or `504`
if it never finishes within the timeout.
...
response). The task-polling `GET` response is plain JSON with no such wrapper. Both are parsed
by the same `_parse_upm_task_response` helper, which strips the wrapper only if present.
...
(403s "Plugins cannot be installed via upload" otherwise) — this platform keeps it enabled
permanently via `-Dupm.plugin.upload.enabled=true`
(`devtools-definition/devtools/confluence/values.yaml`,
`confluence.confluence.additionalJvmArgs`), a deliberate choice since devops-api's plugin install
flow depends on it. The older "Admin → Add-ons → Settings → uncheck 'Prevent users from
installing add-ons'" UI toggle is a separate, additional gate some Confluence versions also
enforce — check both if install still 403s.
