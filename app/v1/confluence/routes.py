...
def get_v1_confluence_router(confluence_client: Any):
    ...
    async def install_confluence_plugin(
        payload: ConfluencePluginInstallRequest,
    ) -> JSONResponse:
        try:
            ...
            await install_plugin(
                confluence_client, plugin_bytes, payload.spec.plugin_name, upm_token
            )
            return SuccessResponse(status="successful")
        except HTTPException as external_error:
            return JSONResponse(
    ...
    @router.post(
        "/plugin/upload", name="upload confluence plugin to MinIO", status_code=200
    )
    async def upload_confluence_plugin(
        payload: ConfluencePluginUploadRequest,
    ) -> JSONResponse:
        try:
            ...
            return JSONResponse(
                {"status": "successful", "plugin_name": payload.spec.plugin_name}
            )
        except HTTPException as external_error:
            return JSONResponse(
                ExceptionResponse(
    ...
    @router.delete(
        "/plugin/{plugin_key:path}", name="uninstall confluence plugin", status_code=200
    )
    ...
    async def export_confluence_space(
        payload: ConfluenceSpaceExportRequest,
    ) -> JSONResponse:
        try:
            job_id, file_name = await trigger_space_export(
                confluence_client, payload.spec
            )
    ...
    async def import_confluence_space(
        payload: ConfluenceSpaceImportRequest,
    ) -> JSONResponse:
        try:
            archive_bytes = await fetch_archive_from_s3(payload.spec)
            job_id = await upload_archive_and_start_restore(
                confluence_client, archive_bytes, payload.spec.archive_name
            )
